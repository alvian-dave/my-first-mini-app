...
const UNISWAP_PAIR = '0x610E319b3A3Ab56A0eD5562927D37c233774ba39'
// -----------------------------------------------------------------------------
// Minimal ABI untuk Uniswap V2 pair
...
export default function TopupModal({
  ...
  const estimatedWr = useMemo(() => {
    ...
    try {
      ...
      const usd_feed = (wldUnits * wldPriceRaw) / BigInt(10 ** 18)
  ...
  // ------------------- Fetch WR price + Uniswap pair -------------------
  useEffect(() => {
    if (!isOpen || !provider || !WRCREDIT_ADDRESS) return
    let mounted = true
  ...
  const buildPermitTypedData = async (tokenAddr: string, owner: string, spender: string, value: bigint, deadline: number) => {
    ...
    const domain = { name, version: '1', chainId, verifyingContract: tokenAddr }
    const types = {
      Permit: [
    ...
    const message = { owner, spender, value: value.toString(), nonce: nonce.toString(), deadline: deadline.toString() }
    return { domain, types, message }
  }

  const signTypedData = async (typed: any) => {
  ...
  const sendTransactionViaPlatform = async (tx: { to: string; data: string; value?: string }) => {
    const world = (window as any).world
    if (world?.sendTransaction) return await world.sendTransaction({ transaction: [{ to: tx.to, data: tx.data, value: tx.value ?? '0x0' }] })
  ...
  return (
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
      <div className="bg-gray-900 text-white rounded-lg p-6 w-full max-w-md relative">
        <button onClick={() => step === 'idle' && onClose()} className="absolute top-3 right-3 text-gray-300" disabled={step !== 'idle'}>✕</button>
