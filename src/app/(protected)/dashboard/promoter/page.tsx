...
export default function PromoterDashboard() {
  ...
  useEffect(() => {
    // fetch on-chain balance safely (guard mounted & session)
    if (!session?.user?.walletAddress) return
    let mounted = true
    const fetchOnChainBalance = async () => {
      try {
        const onChainBal = await getWRCreditBalance(session.user.walletAddress)
        if (mounted) setBalance(Number(onChainBal))
      } catch (err) {
  ...
  useEffect(() => {
    ...
    const loadCampaigns = async () => {
      try {
        ...
        // guard: hanya filter jika session.user.id ada
        const userId = session.user.id ?? null
        const filtered = Array.isArray(data) && userId
          ? (data as UICampaign[]).filter(c => c.createdBy === userId)
          : []
        if (mounted) setCampaigns(filtered)
      } catch (err) {
  ...
  const handleSubmit = async (campaign: BaseCampaign) => {
    try {
      ...
      // refresh campaigns list safely (guard session)
      try {
        ...
        console.error('Failed to refresh campaigns after submit:', err)
        // don't crash — keep existing campaigns
      }
  ...
  const handleMarkFinished = async (id: string) => {
    ...
    try {
      ...
      try {
        ...
        console.error('Failed to refresh campaigns after finish:', err)
      }
